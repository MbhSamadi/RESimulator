...
public class MonitoringState extends LocalState {
    ...
    private Map<String, Interval> deadlockToDeadlockIntervals;
    private List<String> deadlockinit;
    ...
    protected void receiveMonitoringMessageTell(MonitoringMessageMeta meta) {
        ...
        if (reqTransList.isEmpty() && (isOriginTransitionFinal || !isOriginTransitionBlockedOnUnknown)) {
            ...
            if (isFormed) {
                ...
                handleByMethodMonitorMessage(originTransitionMethodName);
                ...
                //}
                List<TransitionItem> transitionItems= transitionTable.findItemByNotFinalMethodName(originTransitionMethodName);
                for (TransitionItem transitionItem : transitionItems) {
                    handleTransitionItemAsk(transitionItem, false, meta.getVectorClock());
                }
                //////
            }
        }
    ...
    protected boolean evaluateFormulation(Transition originTransition, VectorClock messageVC) {
        ...
        List<History> rcvHistList = rcvHistory.get(key);
    ...
    private void receiveMonitoringMessageDeadLock(MonitoringMessageMeta meta) {
        ...
        if (meta.getInitiator().equals(instanceName)) {
            for (Transition targetTransition : meta.getTargetTransitions()) {
                //TransitionClock key2 = new TransitionClock(targetTransition, messageVC);
                if (deadlockToDeadlockIntervals.containsKey(targetTransition.getMethodname())) {
                    deadlockToDeadlockIntervals.get(targetTransition.getMethodname()).setEnd(new Date());
                }
            }
    ...
    protected void handleMonitoringBlockedTargetBlockedAsk(MonitoringMessageMeta meta, Transition targetTransition) {
        String calledMethod = targetTransition.getCalledMethod();
        String orgcalledMethod = meta.getOriginTransition().getCalledMethod();
Transition trs=null;

        if (!deadlockinit.contains(calledMethod+orgcalledMethod) & !orgcalledMethod.equals(calledMethod)){
            ...
            if (finalInitTransitions.size() > 0) {
                ...
                for (TransitionItem transitionItem : finalInitTransitions) {
                    trs=transitionItem.getTransition();
                    if (transitionItem.isPreind()) {
                        presAndVios.add(transitionItem.getPre());
                    }
                    presAndVios.addAll(transitionItem.getVoiIndTransitions());
                }
                Iterator<Transition> iterator = new ArrayList<> (presAndVios).iterator();
                while (iterator.hasNext()) {
                    Transition transition = iterator.next();
                    ...
                    deadlockinit.add(calledMethod+orgcalledMethod);
                    MonitoringMessageMeta messageMeta = new MonitoringMessageMeta(
                            MonitoringMessageType.DEADLOCK,
                            meta.getVectorClock(),
                            trs,
                    ...
                    VectorClock messageVC = meta.getVectorClock();
                    TransitionClock key = new TransitionClock(trs, messageVC);
                    //  intervals.put(key, new Interval(new Date()));
                    deadlockToDeadlockIntervals.put(trs.getMethodname(), new Interval(new Date()));
                }
            }
        }
    ...
    protected void handleByMethodMonitorMessage(String methodname) {
        List<MMItem> handleList = mmList
                .stream()
                .filter(it -> it.gettargetMethod().equals(methodname))
                .collect(Collectors.toList());
    ...
    protected void receiveRegularMessageAsk(String caller, String callee, String methodName,
        ...
        if (!isLastMessage)
            transitionItems = transitionTable.findItemByMethodName(methodName);
        else
            transitionItems = transitionTable.findItemByFinalMethodName(methodName);
    ...
    public Map<String, Interval> getDeadlockToDeadlockIntervals() {
